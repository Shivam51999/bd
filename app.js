...
// exceed safe URL length. Content-Type text/plain keeps this a CORS "simple
// request" (no preflight OPTIONS call), same reasoning as the GET-only
  ...
  body.innerHTML = sorted.map(d => `
    <tr style="cursor:pointer;" onclick='openDealModal(${JSON.stringify(d).replace(/'/g, "&#39;")})'>
      ...
      <td style="white-space:nowrap;">
        <button class="btn btn-outline btn-sm" onclick='event.stopPropagation(); openDealModal(${JSON.stringify(d).replace(/'/g, "&#39;")})'>Edit</button>
        <button class="btn btn-outline btn-sm" style="margin-left:4px;color:var(--red);" onclick='event.stopPropagation(); openDealActivityModal("${d.id}","${escapeHTML(d.parcelName)}")'>+ Activity</button>
        <button class="btn btn-outline btn-sm" style="margin-left:4px;" onclick='event.stopPropagation(); openDocumentModal("${d.id}","${escapeHTML(d.parcelName)}")'>Docs (${documentBadgeCount(d.id)})</button>
      </td>
    </tr>`).join('');
}

// Returns "X/13" count of uploaded docs for this deal across all categories.
// Shown on the Docs button so the BD Manager can see completeness at a glance.
function documentBadgeCount(dealId) {
  if (!STATE.documents) return '0/13';
...
/* ---------------- DEAL ACTIVITY LOG ----------------
   Append-only per-deal work diary. BD Manager logs what happened on
   a lead — site visit, call, meeting, proposal, etc — and these entries
   appear in the CEO Dashboard's deal timeline alongside stage transitions.
   No delete: once logged, an activity is a permanent record. */
...
function setupDealActivityModal() {
  ...
  document.getElementById('dealActivityForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const payload = {
      dealId: document.getElementById('da_dealId').value,
      parcelName: document.getElementById('da_parcelName').value,
      date: document.getElementById('da_date').value,
      activityType: document.getElementById('da_activityType').value,
      summary: document.getElementById('da_summary').value,
      nextFollowupDate: document.getElementById('da_nextFollowupDate').value,
      loggedBy: 'BD Manager',
    };
    if (!payload.summary.trim()) { showToast('Summary is required.', true); return; }
    try {
...
function openDealActivityModal(dealId, parcelName) {
  document.getElementById('da_dealId').value = dealId;
  document.getElementById('da_parcelName').value = parcelName;
  document.getElementById('da_parcelLabel').textContent = parcelName;
  document.getElementById('da_date').value = toISODate(new Date());
  document.getElementById('da_nextFollowupDate').value = '';
  document.getElementById('da_summary').value = '';
  document.getElementById('da_activityType').value = 'Site Visit';
...
/* ---------------- DOCUMENTS (file upload per deal) ----------------
   ...
   Visible on the CEO Dashboard too (read-only links there). */
...
function openDocumentModal(dealId, parcelName) {
  ...
  if (titleEl) titleEl.textContent = 'Documents — ' + parcelName;
...
function renderDocumentChecklist(dealId) {
  ...
  const sections = Object.entries(DOCUMENT_CHECKLIST).map(([catKey, cat]) => {
    const rows = cat.docs.map((docType, idx) => {
      const existing = getLatestDocument(dealId, docType);
      const safeId = `docfile_${catKey}_${idx}`;
      ...
      <div class="doc-row">
        ...
        <div class="doc-row-action">
          <input type="file" id="${safeId}" style="display:none;"
